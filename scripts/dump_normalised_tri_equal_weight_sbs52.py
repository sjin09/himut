...
def load_sbs52_counts(sbs96_file):
    ...
    sbs52_counts = {sbs52: [] for sbs52 in SBS52_LST}
    for line in open(sbs96_file):
        ...
        sbs52_counts[sbs52].append(float(normcount))
    return sbs52_counts


def load_trinucleotide_counts(sbs96_file):
    ...
    for line in open(sbs96_file):
        ...
        tri_counts[tri] = int(ref_tri_count)
    tri_sum = sum(tri_counts.values())
    return tri_sum, tri_counts
...
def dump_sbs52_tri_equal_weights(
    ...
    sbs52_count_lst = load_sbs52_counts(sbs96_file)
    trinucleotide_sum, trinucleotide_counts = load_trinucleotide_counts(sbs96_file)
    with open(sbs52_file, "w") as outfile:
        ...
        for sbs52 in SBS52_LST:
            ...
            sub = f"{ref}>{alt}"
            sbs52_normcount_lst = sbs52_count_lst[sbs52]
            ...
            weighted_sbs52_count = sum(sbs52_normcount_lst) * sbs52_tri_weight
            print(
                sub,
                SBS96_TO_TRI[sbs52],
                sbs52,
                weighted_sbs52_count,
                ",".join([str(sbs52_normcount) for sbs52_normcount in sbs52_normcount_lst]),
                sbs52_tri_weight,
                sep="\t",
                file=outfile
            )
