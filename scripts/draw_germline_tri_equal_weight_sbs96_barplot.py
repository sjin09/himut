...
def parse_args(args):
    ...
    parser.add_argument(
        ...
        required=False,
        action="store_true",
        help="read and reference is from the same sample"
    )
    parser.add_argument(
        "-t",
        "--threads",
...
class ExpandedVariantRecord:
    ...
    @property
    def samples(self) -> pysam.VariantRecord.samples:
...
def load_sbs96_counts(
    ...
    vcf_path: Path,
    ref_path: Path,
    is_reference_sample: bool
) -> Dict[str, int]:
    sample = get_sample(vcf_path)
    ...
    for variant in variant_records:
        ...
        if not xvariant.is_biallelic:
            continue
        if len(xvariant.ref) == len(xvariant.alts[0]) == 1:
            sample_gt = xvariant.samples[sample]["GT"]
            if is_reference_sample:
                if sample_gt == (0, 1):
                    sbs96 = get_sbs96(xvariant, reference_sequence_lookup)
                    sbs96_per_chrom[variant.chrom][sbs96] += 1
            else:
                if sample_gt == (0, 1) or sample_gt == (1, 1):
                    sbs96 = get_sbs96(xvariant, reference_sequence_lookup)
                    sbs96_per_chrom[variant.chrom][sbs96] += 1
...
def write_tri_equal_weight_sbs96_counts(
    ...
    sample = get_sample(vcf_path)
    chroms = load_loci(region, region_list)
    sbs96_counts = load_sbs96_counts(chroms, vcf_path, ref_path, is_reference_sample)
    tricount_lookup = get_target_tricounts(chroms, ref_path, threads)
    ...
    draw_sbs96_barplot(out_path, sample, pdf_path)


def main():
    options = parse_args(sys.argv)
    write_tri_equal_weight_sbs96_counts(
        options.vcf,
        options.ref,
        options.region,
        options.region_list,
