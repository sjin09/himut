...
SBS52_TRI_LST = list(set([SBS96_TO_TRI[sbs52] for sbs52 in SBS52_LST]))
SBS52_TRI_COUNT = len(SBS52_TRI_LST)
SBS52_TRI_WEIGHT = 1/SBS52_TRI_COUNT
...
def load_sbs52_counts(sbs96_file):
    ...
    sbs52_to_count = defaultdict(lambda: 0)
    for line in open(sbs96_file):
        ...
        sbs52_to_count[sbs52] += int(normcount)
    return sbs52_to_count
...
def dump_sbs52_tri_equal_weights(
    ...
    tri_freq = load_tri_freq(sbs96_file)
    with open(sbs52_file, "w") as outfile:
        ...
        for line in open(sbs96_file).readlines():
            ...
            sbs52_count[sbs52].append(float(normcount))
            # tri_equal_weight_normcounts     
       
        for sbs52 in SBS52_LST:
            _ubase, _, ref, _, alt, _, _dbase = list(sbs52)
            sub = f"{ref}>{alt}"
            tri = SBS96_TO_TRI[sbs52]
            normcount_lst = sbs52_count[sbs52]
            tri_equal_weight = (tri_freq[tri]/SBS52_TRI_WEIGHT)
            tri_equal_weight_normcount = sum(normcount_lst)/(tri_freq[tri]/SBS52_TRI_WEIGHT)
            print(
                sub,
                SBS96_TO_TRI[sbs52],
                sbs52,
                tri_equal_weight_normcount,
                ",".join([str(normcount) for normcount in normcount_lst]),
                tri_equal_weight,
                sep="\t",
                file=outfile
            )
...
def main():
    ...
    dump_sbs52_tri_equal_weights(options.sbs96, options.sbs52)
    sys.exit(0)
