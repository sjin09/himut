...
def get_som_pD(
    ...
    for base in himut.util.base_lst:
        ...
        if (base == ref or base == alt): ## hom
            hom_gt_p_lst.extend([himut.gtlib.get_one_minus_epsilon(base_bq) for base_bq in base_bq_lst])
            het_gt_p_lst.extend([himut.gtlib.get_one_half_minus_epsilon(base_bq) for base_bq in base_bq_lst])
        else: ## error
            error_p_lst = [himut.gtlib.get_epsilon(base_bq/3) for base_bq in base_bq_lst]
...
def get_log2_som_pD(
    ...
    for base in himut.util.base_lst:
        base_bq_lst = allele2bq_lst[himut.util.base2idx[base]]
        if (base == ref or base == alt): ## hom
            hom_gt_pD += sum([get_log2_one_minus_epsilon(base_bq) for base_bq in base_bq_lst])
            het_gt_pD += sum([get_log2_one_half_minus_epsilon(base_bq) for base_bq in base_bq_lst])
        else: ## error
